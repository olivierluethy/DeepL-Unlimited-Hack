...
Hier ist die vollständige Überarbeitung. Der Button nimmt nun den letzten konvertierten Text und setzt ihn oben ins Feld ein, damit du ihn sofort weiterbearbeiten oder erneut senden kannst.
  ...
  function loadHistory() {
    chrome.storage.local.get({ verlauf: [] }, (result) => {
      ...
      <!-- ÄNDERUNG: w-100 und d-flex -->
      <div class="d-flex gap-3 mb-2 w-100" style="table-layout: fixed;">
        
        <!-- Linke Spalte: Original (fest 50%) -->
        <div class="w-50" style="overflow-wrap: break-word; min-width: 0;">
          <strong class="d-block small text-secondary">Original:</strong>
          <pre class="mb-0 text-wrap" style="white-space: pre-wrap; word-break: break-word; background: #f8f9fa; padding: 8px; border-radius: 4px;">${sanitize(entry.original)}</pre>
        </div>
        
        <!-- Rechte Spalte: Converted (fest 50%) -->
        <div class="w-50" style="overflow-wrap: break-word; min-width: 0;">
          <strong class="d-block small text-primary">Converted:</strong>
          <pre class="mb-0 text-wrap" style="white-space: pre-wrap; word-break: break-word; background: #f8f9fa; padding: 8px; border-radius: 4px;">${sanitize(entry.translated)}</pre>
          
          <!-- Buttons bleiben sauber drunter -->
          
          <div class="d-flex flex-nowrap gap-1 mt-2 align-items-center">
